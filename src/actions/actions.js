...
export const newText = (mode, words) => {
  ...
  return async (dispatch) => {
       ...
       const wiki = await axios
        .get(`https://api.allorigins.win/get?url=${encodeURIComponent(wikiURL)}`)
       ...
       const combine = text => flatten(chunk(2, text).map(x =>
